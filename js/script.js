...
function getRandomColor(){return "#000000".replace(/0/g,function(){return (~~(Math.random()*16)).toString(16);});}
const circle = {
...
function getCurrentOptions(object, options){
    ...
    const currentOps = optionsArray.reduce((data, option)=>{
...
function drawCirlce (context, circle, options, lines) {
    ...
    context.beginPath();
    context.arc(...circle.center, circle.radius, 0, 2 * Math.PI, false);
    const currentOps = getCurrentOptions(context, options)
    setOptions(context,options)
    context.stroke();
    if(lines){
        arry(lines).forEach(angle =>{
            context.beginPath();
            context.moveTo(...circle.center);
            context.lineTo(...getArcCoords(circle,angle)(circle.radius));
            context.closePath();
            context.stroke();
        })
    }
...
function printGrapiph(data){
    const arc = 360 / data.length
        ...
        const fullCoord = data.map((item, index)=> {return {coords: getArcCoords(circle, arc * index)(circle.radius), Nombre: item.Nombre, arc:arc * index}} )
...
const captions = [
    {
        name: 'Autoevaluacion',
        coords: [60,(containerHeight * .8) ]
    },
    {
        name: 'Requerida',
        coords: [60,(containerHeight * .8 + 15) ]
    },
    {
        name: 'Evaluacion',
        coords: [60,(containerHeight * .8 + 30) ]
    }
]
